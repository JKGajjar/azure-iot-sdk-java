...
public final class AmqpsIotHubConnection extends BaseHandler implements IotHubTransportConnection
{
    ...
    public void open(Queue<DeviceClientConfig> deviceClientConfigs, ScheduledExecutorService scheduledExecutorService) throws TransportException
    {
        ...
        sasTokenRenewalHandler = new SasTokenRenewalHandler(this.amqpsSessionManager, this.deviceClientConfig);
     ...
     */
    private Integer sendMessage(Message message, MessageType messageType, String deviceId) throws TransportException
    {
        logger.LogDebug("Entered in method %s", logger.getMethodName());
    ...
    private void processMessagesToSend()
    {
        ...
        while (message != null && messagesAttemptedToBeProcessed < 1000 && lastDeliveryTag >= 0)
        {
            ...
            try
            {
                lastDeliveryTag = this.sendMessage(amqpsConvertToProtonReturnValue.getMessageImpl(), amqpsConvertToProtonReturnValue.getMessageType(), message.getConnectionDeviceId());
            }
            catch (TransportException e)
            {
                if (e.isRetryable())
